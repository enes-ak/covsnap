...
def parse_region(region_str: str) -> tuple[str, int, int]:
    ...
    if not m:
        raise ValueError(
            f"Invalid region format: '{region_str}'. Expected: contig:start-end (e.g. chr17:43044295-43125482)"
        )
    contig = m.group(1)
    start_1 = int(m.group(2).replace(",", ""))
    end_1 = int(m.group(3).replace(",", ""))
    if start_1 >= end_1:
        raise ValueError(f"Invalid region: start ({start_1}) must be less than end ({end_1}).")
